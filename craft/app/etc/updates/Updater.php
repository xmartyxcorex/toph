...
class Updater
{
	...
	public function processDownload($md5)
	{
		...
		if (!empty($errors))
		{
			throw new Exception(Craft::t('Your server does not meet the following minimum requirements for Craft to run:')."\n\n".$this->_markdownList($errors));
		}
		...
		{
			throw new Exception(Craft::t('Craft needs to be able to write to the following paths, but can’t:')."\n\n".$this->_markdownList($writableErrors));
		}
	...
	private function _validateNewRequirements($unzipFolder)
	{
		...
		{
			throw new Exception(Craft::t('Craft needs to be able to write to your craft/app/etc/requirements folder and cannot. Please check your [permissions]({url}).', array('url' => 'http://buildwithcraft.com/docs/updating#one-click-updating')));
		}
